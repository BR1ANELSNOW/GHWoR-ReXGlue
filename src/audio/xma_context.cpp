...
constexpr size_t kMultistreamLaneCapacity = 512;
constexpr uint32_t kMultistreamBufferPacketCount = 16;
static std::array<uint8_t, kMultistreamLaneCapacity> multistream_packet_lanes = {};
static std::array<bool, kMultistreamLaneCapacity> multistream_lane_initialized = {};
...
void XmaContext::ResetDecoderState() {
  ...
  const size_t lane_index =
      std::min<size_t>(static_cast<size_t>(id()), kMultistreamLaneCapacity - 1);
  multistream_packet_lanes[lane_index] = 0;
  multistream_lane_initialized[lane_index] = false;
}
...
const uint8_t* XmaContext::GetNextPacket(XMA_CONTEXT_DATA* data, uint32_t next_packet_index,
  ...
  // Preserve the packet position when an interleaved XMA stream crosses to
  // the next buffer. Multiple hardware contexts may start one packet apart;
  // returning only the base address makes all of them collapse onto packet 0.
  uint32_t carried_packet_index =
      next_packet_index - current_input_packet_count;
  if (carried_packet_index == 0 &&
      current_input_packet_count == kMultistreamBufferPacketCount) {
    const size_t lane_index =
        std::min<size_t>(static_cast<size_t>(id()), kMultistreamLaneCapacity - 1);
    carried_packet_index = multistream_packet_lanes[lane_index];
  }
  return memory()->TranslatePhysical(next_buffer_address) +
         (carried_packet_index * kBytesPerPacket);
}
...
void XmaContext::Decode(XMA_CONTEXT_DATA* data) {
  ...
  // The first buffer supplied for an interleaved stream is shortened by its
  // packet lane (16, 15, 14...). Remember that lane after later buffers are
  // normalized to the full 16 packets.
  const size_t multistream_lane_index =
      std::min<size_t>(static_cast<size_t>(id()), kMultistreamLaneCapacity - 1);
  if (!multistream_lane_initialized[multistream_lane_index] &&
      current_input_packet_count > 1 &&
      current_input_packet_count <= kMultistreamBufferPacketCount) {
    multistream_packet_lanes[multistream_lane_index] = static_cast<uint8_t>(
        kMultistreamBufferPacketCount - current_input_packet_count);
    multistream_lane_initialized[multistream_lane_index] = true;
  }
  ...
  if (skip_count == 0xFF) {
    ...
    if (next_input_offset == kBitsPerPacketHeader) {
      uint32_t carried_packet_index = 0;
      if (current_input_packet_count == kMultistreamBufferPacketCount) {
        carried_packet_index = multistream_packet_lanes[multistream_lane_index];
      }
      SwapInputBuffer(data);
      if (data->IsAnyInputBufferValid()) {
        const uint32_t next_buffer_packet_count =
            data->GetCurrentInputBufferPacketCount();
        if (carried_packet_index >= next_buffer_packet_count) {
          SwapInputBuffer(data);
          return;
        }
        const uint8_t* next_lane_packet =
            memory()->TranslatePhysical(data->GetCurrentInputBufferAddress()) +
            (carried_packet_index * kBytesPerPacket);
        const uint32_t next_lane_frame_offset =
            xma::GetPacketFrameOffset(next_lane_packet);
        if (next_lane_frame_offset > kMaxFrameSizeinBits) {
          SwapInputBuffer(data);
          return;
        }
        next_input_offset =
            (carried_packet_index * kBitsPerPacket) + next_lane_frame_offset;
      }
    }
  ...
  if (bits_to_copy == 0) {
    ...
    if (data->IsAnyInputBufferValid()) {
      ...
      if (carried_packet_index >= next_buffer_packet_count) {
        REXAPU_ERROR(
            "XmaContext {}: Recovery lane {} exceeds next buffer packet count {}",
            id(), carried_packet_index, next_buffer_packet_count);
        SwapInputBuffer(data);
        return;
      }

      const uint8_t* recovery_packet =
          memory()->TranslatePhysical(data->GetCurrentInputBufferAddress()) +
          (carried_packet_index * kBytesPerPacket);
      const uint32_t recovery_frame_offset =
          xma::GetPacketFrameOffset(recovery_packet);
      if (recovery_frame_offset > kMaxFrameSizeinBits) {
        REXAPU_ERROR(
            "XmaContext {}: Invalid recovery frame offset {} in lane {}",
            id(), recovery_frame_offset, carried_packet_index);
        SwapInputBuffer(data);
        return;
      }
      data->input_buffer_read_offset =
          (carried_packet_index * kBitsPerPacket) + recovery_frame_offset;
    }
    return;
  }
  ...
  if (next_input_offset == kBitsPerPacketHeader) {
    // Full 16-packet buffers normalize interleaved streams. Restore the
    // context's original packet lane when crossing into the next buffer.
    uint32_t carried_packet_index =
        next_packet_index >= current_input_packet_count
            ? next_packet_index - current_input_packet_count
            : 0;
    if (current_input_packet_count == kMultistreamBufferPacketCount &&
        multistream_lane_initialized[multistream_lane_index]) {
      const uint32_t stored_lane =
          multistream_packet_lanes[multistream_lane_index];
      carried_packet_index = stored_lane;
    }

    SwapInputBuffer(data);
    if (data->IsAnyInputBufferValid()) {
      const uint32_t carried_buffer_packet_count =
          data->GetCurrentInputBufferPacketCount();
      if (carried_packet_index >= carried_buffer_packet_count) {
        REXAPU_ERROR(
            "XmaContext {}: Carried packet index {} exceeds next buffer "
            "packet count {}",
            id(), carried_packet_index, carried_buffer_packet_count);
        SwapInputBuffer(data);
        return;
      }

      const uint8_t* carried_packet =
          memory()->TranslatePhysical(data->GetCurrentInputBufferAddress()) +
          (carried_packet_index * kBytesPerPacket);
      const uint32_t carried_frame_offset =
          xma::GetPacketFrameOffset(carried_packet);

      next_input_offset =
          (carried_packet_index * kBitsPerPacket) + carried_frame_offset;

      if (carried_frame_offset > kMaxFrameSizeinBits) {
        SwapInputBuffer(data);
        return;
      }
    }
