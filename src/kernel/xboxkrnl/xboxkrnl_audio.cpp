...
// ReXGlue currently renders to a stereo host stream. Keep a stable Xbox 360
// speaker configuration for queries, while recording override requests for
// compatibility and diagnostics.
constexpr uint32_t kDefaultSpeakerConfig = 0x00010001;
uint32_t g_speaker_config = kDefaultSpeakerConfig;
uint32_t g_speaker_override = 0;
...
u32 XAudioGetSpeakerConfig_entry(mapped_u32 config_ptr) {
  ...
  *config_ptr = g_speaker_config;
  return X_ERROR_SUCCESS;
}

u32 XAudioSetSpeakerConfig_entry(u32 config) {
  g_speaker_config = config;
  REXKRNL_DEBUG("XAudioSetSpeakerConfig: config=0x{:08X}",
                static_cast<uint32_t>(config));
  return X_ERROR_SUCCESS;
}

u32 XAudioOverrideSpeakerConfig_entry(u32 config) {
  // The override value is a policy flag used by titles, not a host channel
  // matrix. Store it separately so GetSpeakerConfig remains stable.
  g_speaker_override = config;
  REXKRNL_DEBUG("XAudioOverrideSpeakerConfig: override=0x{:08X}",
                static_cast<uint32_t>(g_speaker_override));
  return X_ERROR_SUCCESS;
}
...
REX_EXPORT(__imp__XAudioSetSpeakerConfig,
           rex::kernel::xboxkrnl::XAudioSetSpeakerConfig_entry)
REX_EXPORT(__imp__XAudioOverrideSpeakerConfig,
           rex::kernel::xboxkrnl::XAudioOverrideSpeakerConfig_entry)
