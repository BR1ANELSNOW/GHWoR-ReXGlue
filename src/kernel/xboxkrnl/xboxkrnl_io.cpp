...
u32 NtCancelIoFile_entry(
    u32 file_handle,
    ppc_ptr_t<X_IO_STATUS_BLOCK> io_status_block) {
  X_STATUS result = X_STATUS_SUCCESS;

  // ReXGlue currently completes NtReadFile synchronously even for handles
  // opened as asynchronous. Therefore there is no host request still pending
  // here, but Xbox callers still require the cancellation IOSB to be completed.
  auto file =
      REX_KERNEL_OBJECTS()->LookupObject<XFile>(file_handle);
...
u32 IoDismountVolumeByFileHandle_entry(u32 handle) {
  // Mounted game volumes are owned by ReXGlue's virtual file system rather
  // than a removable host volume. Acknowledge the request without tearing down
  // the shared mount used by the running title.
  auto file = REX_KERNEL_OBJECTS()->LookupObject<XFile>(handle);
  if (!file) {
    // Dismount is commonly part of cleanup, so an already-closed or untracked
    // handle is treated as an idempotent success.
    REXKRNL_DEBUG(
        "IoDismountVolumeByFileHandle({:#x}) - handle already closed; treating as dismounted",
        static_cast<uint32_t>(handle));
    return X_STATUS_SUCCESS;
  }

  REXKRNL_DEBUG("IoDismountVolumeByFileHandle({:#x}) - virtual dismount acknowledged",
                static_cast<uint32_t>(handle));
  return X_STATUS_SUCCESS;
}
...
REX_EXPORT(__imp__NtCancelIoFile, rex::kernel::xboxkrnl::NtCancelIoFile_entry)
