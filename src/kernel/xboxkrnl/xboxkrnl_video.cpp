...
std::pair<uint32_t, uint32_t> CalculateScaledAspectRatio(uint32_t fb_x,
                                                         uint32_t fb_y,
                                                         bool forced_widescreen) {
  if (!fb_x || !fb_y) {
    return {16, 9};
  }
  ...
  uint32_t factor = std::gcd(fb_x, fb_y);
  return {fb_x / factor, fb_y / factor};
}
...
void VdGetCurrentDisplayInformation_entry(ppc_ptr_t<X_DISPLAY_INFO> display_info) {
  ...
  // Xenia cdd6049 exposes the physical 360 display-information overscan.
...
void VdQueryVideoMode(X_VIDEO_MODE* video_mode) {
  // Xenia cdd6049 exposes the console video mode, not the Windows monitor mode.
  // The working GHWoR Canary configuration is 720p progressive at 60 Hz.
...
u32 VdInitializeScalerCommandBuffer_entry(
  ...
  // Xenia cdd6049 keeps the hardware scaler aspect ratio as GPU state and
  // consumes it in IssueSwap. GHWoR's active 720p mode is widescreen.
  uint32_t fb_x = (scaled_output_wh >> 16) & 0xFFFF;
  uint32_t fb_y = scaled_output_wh & 0xFFFF;
  const bool forced_widescreen = false;
