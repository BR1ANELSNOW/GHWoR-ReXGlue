...
u32 NtQueryVolumeInformationFile_entry(u32 file_handle,
  ...
  switch (info_class) {
    ...
    case XFileFsDeviceInformation: {
      ...
      // The SDK enum currently omits FILE_DEVICE_DISK, whose documented
      // device-type value is 0x00000007. Keep the strongly typed assignment so
      // endian_store<X_FILE_DEVICE_TYPE> accepts it.
      constexpr X_FILE_DEVICE_TYPE kFileDeviceDisk =
          static_cast<X_FILE_DEVICE_TYPE>(0x00000007);
      info->device_type = kFileDeviceDisk;
      info->characteristics = 0;
      out_length = sizeof(X_FILE_FS_DEVICE_INFORMATION);

      REXKRNL_DEBUG("XFileFsDeviceInformation: device='{}', type={:#x}",
                    file->device()->name(), static_cast<uint32_t>(kFileDeviceDisk));
      break;
    }
    default: {
      // Unsupported, for now.
