...
u32 XMAInitializeContext_entry(mapped_void context_ptr, ppc_ptr_t<XMA_CONTEXT_INIT> context_init) {
  ...
  // WoR XMA buffer probe: records the original guest pointers and the
  // translated physical addresses used by every decoder context.
  REXKRNL_WARN(
      "XmaBufferInit ctx={:08X} "
      "in0_guest={:08X} in0_phys={:08X} packets0={} "
      "in1_guest={:08X} in1_phys={:08X} packets1={} "
      "read_offset={} out_guest={:08X} out_phys={:08X} "
      "out_blocks={} channels_field={} sample_rate={}",
      context_ptr.guest_address(), input_buffer_0_guest_ptr,
      input_buffer_0_physical_address,
      uint32_t(context_init->input_buffer_0_packet_count),
      input_buffer_1_guest_ptr, input_buffer_1_physical_address,
      uint32_t(context_init->input_buffer_1_packet_count),
      uint32_t(context_init->input_buffer_read_offset),
      output_buffer_guest_ptr, output_buffer_physical_address,
      uint32_t(context_init->output_buffer_block_count),
      uint32_t(context_init->channel_count),
      uint32_t(context_init->sample_rate));
...
u32 XMASetInputBuffer0_entry(mapped_void context_ptr, mapped_void buffer, u32 packet_count) {
  ...
  // WoR XMA buffer probe: records subsequent buffer 0 replacements.
  REXKRNL_WARN(
      "XmaBufferSet0 ctx={:08X} guest={:08X} phys={:08X} packets={}",
      context_ptr.guest_address(), buffer.guest_address(),
...
u32 XMASetInputBuffer1_entry(mapped_void context_ptr, mapped_void buffer, u32 packet_count) {
  ...
  // WoR XMA buffer probe: records subsequent buffer 1 replacements.
  REXKRNL_WARN(
      "XmaBufferSet1 ctx={:08X} guest={:08X} phys={:08X} packets={}",
      context_ptr.guest_address(), buffer.guest_address(),
      buffer_physical_address, packet_count);
