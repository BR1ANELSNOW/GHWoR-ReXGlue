...
REXCVAR_DECLARE(int32_t, ghwor_keyboard_instrument);
...
inline bool GhWoRVirtualGhDrumsActive() {
  // El teclado P1 conserva su identidad aunque haya un joystick SDL en P2.
  return REXCVAR_GET(ghwor_keyboard_instrument) == 1;
}

inline void GhWoRForceFiveLaneCaps(X_INPUT_CAPABILITIES* caps) {
  if (!caps) {
    return;
  }

  // GHWOR KEYBOARD GH DRUM PHYSICAL CLONE v18
  // Exact capabilities captured from the physical Xbox 360 Guitar Hero drums.
  caps->type = 0x01;
  caps->sub_type = 0x08;
  caps->flags = 0x0006;

  caps->gamepad.buttons = 0xF37F;
  caps->gamepad.left_trigger = 0;
  caps->gamepad.right_trigger = 0;
  caps->gamepad.thumb_lx = static_cast<int16_t>(5168);
  caps->gamepad.thumb_ly = static_cast<int16_t>(2053);
  caps->gamepad.thumb_rx = static_cast<int16_t>(1);
  caps->gamepad.thumb_ry = static_cast<int16_t>(-64);

  caps->vibration.left_motor_speed = 0;
  caps->vibration.right_motor_speed = 0;

  static bool ghwor_keyboard_physical_clone_v18_logged = false;
  if (!ghwor_keyboard_physical_clone_v18_logged) {
    ghwor_keyboard_physical_clone_v18_logged = true;
    REXKRNL_WARN(
        "GHWOR KEYBOARD GH DRUM PHYSICAL CLONE v18: "
        "type=0x01 subtype=0x08 flags=0x0006 caps=0xF37F "
        "idle=0x0040 axes=5168/2053/1/-64");
  }
}

inline void GhWoRForceFiveLaneState(X_INPUT_STATE* state) {
  if (!state) {
    return;
  }
  ...
  // Physical GH drums keep LEFT_THUMB asserted while idle.
  buttons |= 0x0040;

  const bool green =
      (buttons & rex::input::X_INPUT_GAMEPAD_A) != 0;
  const bool red =
      (buttons & rex::input::X_INPUT_GAMEPAD_B) != 0;
  const bool blue =
      (buttons & rex::input::X_INPUT_GAMEPAD_X) != 0;
  const bool yellow =
      (buttons & rex::input::X_INPUT_GAMEPAD_Y) != 0;
  const bool kick =
      (buttons & rex::input::X_INPUT_GAMEPAD_LEFT_SHOULDER) != 0;
  const bool orange =
      (buttons & rex::input::X_INPUT_GAMEPAD_RIGHT_SHOULDER) != 0;
  ...
  // Same axis shape measured on the real kit and already proven with joystick.
  state->gamepad.thumb_ly =
      red ? static_cast<int16_t>(9216)
          : (green ? static_cast<int16_t>(64) : 0);

  state->gamepad.thumb_rx =
      blue ? static_cast<int16_t>(8192)
           : (yellow ? static_cast<int16_t>(32) : 0);

  state->gamepad.thumb_ry =
      kick ? static_cast<int16_t>(12288)
           : (orange ? static_cast<int16_t>(32) : 0);
}
...
u32 XamInputGetCapabilities_entry(u32 user_index, u32 flags, ppc_ptr_t<X_INPUT_CAPABILITIES> caps) {
  ...
  if (XSUCCEEDED(result) && actual_user_index == 0 &&
      GhWoRVirtualGhDrumsActive()) {
    GhWoRForceFiveLaneCaps(caps);
  }
  // v16: no joystick capability probe here; SDL/InputSystem owns joystick identity.
  return result;
}
...
u32 XamInputGetState_entry(u32 user_index, u32 flags, ppc_ptr_t<X_INPUT_STATE> input_state) {
  ...
  if (XSUCCEEDED(result) && actual_user_index == 0 &&
      GhWoRVirtualGhDrumsActive()) {
    GhWoRForceFiveLaneState(input_state);
  }
  return result;
}
...
i32 XamUserGetDeviceContext_entry(u32 user_index, u32 unk,
                                  mapped_u32 out_ptr) {
  *out_ptr = 0;

  auto* is = input_system();
  uint32_t actual_user_index = user_index;
  i32 result = X_E_DEVICE_NOT_CONNECTED;

  if ((user_index & 0xFF) == 0xFF) {
    for (uint32_t i = 0; i < 4; ++i) {
      X_INPUT_CAPABILITIES caps = {};
      if (XSUCCEEDED(
              is->GetCapabilities(i, XINPUT_FLAG_GAMEPAD, &caps))) {
        actual_user_index = i;
        result = X_E_SUCCESS;
        break;
      }
  ...
  if ((user_index & 0xFF) != 0xFF) {
    X_INPUT_CAPABILITIES caps = {};
    if (XSUCCEEDED(is->GetCapabilities(
            actual_user_index, XINPUT_FLAG_GAMEPAD, &caps))) {
      result = X_E_SUCCESS;
    }
  }

  // El teclado bateria conserva el contexto sintetico de P1.
  // El joystick P2 usa su identidad SDL/InputSystem normal.
  if (result == X_E_SUCCESS && actual_user_index == 0 &&
      GhWoRVirtualGhDrumsActive()) {
    if (unk == 1) {
      *out_ptr = 0x50000000u;
    }
  ...
  static bool ghwor_dual_player_v19_fix2_logged = false;
  if (!ghwor_dual_player_v19_fix2_logged &&
      result == X_E_SUCCESS) {
    ghwor_dual_player_v19_fix2_logged = true;
    REXKRNL_WARN(
        "GHWOR DUAL PLAYER ROUTING v19 FIX2 ACTIVE: "
        "keyboard=P1 joystick=P2");
  }
