...
u32 XamContentAggregateCreateEnumerator_entry(u64 xuid, u32 device_id, u32 content_type, u32 unk3,
  ...
  REXKRNL_WARN(
      "GHWOR AGG CREATE: xuid={:016X} device=0x{:08X} type=0x{:08X} unk3=0x{:08X}",
      xuid, device_id, content_type, unk3);
  ...
  if (!device_info || device_info->device_type == DeviceType::HDD) {
    ...
    for (auto& title_id : title_ids) {
      ...
      REXKRNL_WARN(
          "GHWOR AGG USER: title=0x{:08X} requested_xuid={:016X} items={}",
          title_id, xuid, content_datas.size());
      ...
      if (userxuid != 0) {
        ...
        REXKRNL_WARN(
            "GHWOR AGG COMMON: title=0x{:08X} items={}",
            title_id, common_datas.size());
  ...
  // Marketplace DLC comes exclusively from the Xbox 360-style HDD Content tree.
  if (content_type_enum != XContentType::kMarketplaceContent &&
      (!device_info || device_info->device_type == DeviceType::ODD)) {
  ...
  REXKRNL_WARN("GHWOR AGG READY: items={}", e->item_count());
  *handle_out = e->handle();
