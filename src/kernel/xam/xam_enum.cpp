...
uint32_t xeXamEnumerate(uint32_t handle, uint32_t flags, mapped_void buffer_ptr,
  ...
  if (items_returned) {
    ...
    *items_returned = result == X_ERROR_SUCCESS ? item_count : 0;
    ...
    // GHWOR: XamEnumerate is effectively synchronous here.
    // Complete the X_OVERLAPPED immediately, but preserve the
    // asynchronous API return value (X_ERROR_IO_PENDING).
    uint32_t extended_error = 0;
    uint32_t item_count = 0;
    X_RESULT result = run(extended_error, item_count);

    REXKRNL_WARN(
        "GHWOR XAM ENUM IMMEDIATE: overlapped={:08X} result={:08X} "
        "extended={:08X} items={}",
        overlapped_ptr, static_cast<uint32_t>(result),
        extended_error, item_count);

    REX_KERNEL_STATE()->CompleteOverlappedImmediateEx(
        overlapped_ptr, result, extended_error, item_count);
