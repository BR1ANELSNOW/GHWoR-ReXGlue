...
X_HRESULT XamApp::DispatchMessageSync(uint32_t message, uint32_t buffer_ptr,
  ...
  switch (message) {
    case 0x0002000E: {
      ...
      // GHWOR AGG BUFFER GUARD FIX1
      const uint32_t ghwor_agg_buffer_size =
          static_cast<uint32_t>(data->buffer_size);

      const uint32_t ghwor_agg_required_size =
          static_cast<uint32_t>(sizeof(XCONTENT_AGGREGATE_DATA));

      REXKRNL_WARN(
          "GHWOR AGG BUFFER: guest={} required={} ptr={:08X} length_ptr={:08X}",
          ghwor_agg_buffer_size,
          ghwor_agg_required_size,
          static_cast<uint32_t>(data->buffer_ptr),
          static_cast<uint32_t>(data->length_ptr));

      if (ghwor_agg_buffer_size < ghwor_agg_required_size) {
        REXKRNL_ERROR(
            "GHWOR AGG BUFFER TOO SMALL: guest={} required={}",
            ghwor_agg_buffer_size,
            ghwor_agg_required_size);
      ...
      REXKRNL_WARN(
          "GHWOR AGG WRITE: result={:08X} items={} guest={} required={}",
      ...
      if (result == X_ERROR_SUCCESS && item_count >= 1) {
        auto returned_item =
            reinterpret_cast<XCONTENT_AGGREGATE_DATA*>(buffer);

        const uint32_t returned_type =
            static_cast<uint32_t>(
                static_cast<XContentType>(
                    returned_item->content_type));

        REXKRNL_WARN(
            "GHWOR AGG ITEM: device=0x{:08X} type=0x{:08X} title=0x{:08X} file='{}'",
            static_cast<uint32_t>(returned_item->device_id),
            returned_type,
            static_cast<uint32_t>(returned_item->title_id),
            returned_item->file_name());
      }
