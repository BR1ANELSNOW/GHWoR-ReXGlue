...
static bool IsSingleUserSignedIn(u32 user_index) { return user_index == 0; }

static uint32_t GetSingleUserCachedFlags() {
  // Match the account flag layout used by Canary: country in bits 8-15,
  // subscription tier in bits 20-23, language in bits 25-29.
  const uint32_t country = REXCVAR_GET(user_country) & 0xFF;
  const uint32_t language = REXCVAR_GET(user_language) & 0x1F;
  const uint32_t subscription_tier = 6;  // Gold, preserving ReX's prior behavior.
  return (country << 8) | (subscription_tier << 20) | (language << 25);
}
...
i32 XamUserGetSigninInfo_entry(u32 user_index, u32 flags, ppc_ptr_t<X_USER_SIGNIN_INFO> info) {
  ...
  const auto& user_profile = REX_KERNEL_STATE()->user_profile();
  if (!flags || (flags & 0x2)) {  // X_USER_GET_SIGNIN_INFO_OFFLINE_XUID_ONLY
...
uint32_t XamUserReadProfileSettingsEx(uint32_t title_id, uint32_t user_index, uint32_t xuid_count,
  ...
  if (!xuid_count) {
    ...
    // ReX currently exposes one signed-in profile. Keep the API behavior
    // deterministic instead of asserting on guest input.
    if (xuid_count != 1 || !xuids) {
      return X_ERROR_INVALID_PARAMETER;
    }
    const auto& user_profile = REX_KERNEL_STATE()->user_profile();
    if (static_cast<uint64_t>(xuids[0]) != user_profile->xuid()) {
  ...
  for (uint32_t n = 0; n < setting_count; ++n) {
    ...
    if (!setting) {
      REXKRNL_WARN("XamUserReadProfileSettingsEx skipping unimplemented setting {:08X}", setting_id);
      out_header->setting_count = static_cast<uint32_t>(out_header->setting_count) - 1;
      continue;
    }
...
u32 XamUserReadProfileSettingsEx_entry(u32 title_id, u32 user_index, u32 xuid_count,
                                       ...
                                       mapped_u32 buffer_size_ptr, mapped_u32 alternate_buffer_size_ptr,
  ...
  return XamUserReadProfileSettingsEx(title_id, user_index, xuid_count, xuids, setting_count,
                                      setting_ids, 0, alternate_buffer_size_ptr, buffer_ptr, overlapped);
}
  ...
  for (uint32_t n = 0; n < setting_count; ++n) {
    ...
    switch (setting_type) {
      ...
      case UserProfile::Setting::Type::BINARY: {
        ...
        std::vector<uint8_t> bytes(binary_size, 0);
        if (setting.data.binary.ptr && binary_size) {
      ...
      case UserProfile::Setting::Type::WSTRING: {
        std::u16string value;
        const uint32_t byte_size = setting.data.unicode.size;
        if (setting.data.unicode.ptr && byte_size) {
          auto chars = REX_KERNEL_MEMORY()->TranslateVirtual<be<uint16_t>*>(setting.data.unicode.ptr);
          if (!chars || (byte_size & 1)) {
            return X_ERROR_INVALID_PARAMETER;
          }
          const size_t char_count = byte_size / sizeof(uint16_t);
          value.reserve(char_count);
          for (size_t i = 0; i < char_count; ++i) {
            char16_t c = static_cast<char16_t>(static_cast<uint16_t>(chars[i]));
            if (!c) break;
            value.push_back(c);
          }
        }
      ...
      case UserProfile::Setting::Type::DATETIME:
        ...
        break;
      default:
        REXKRNL_WARN("XamUserWriteProfileSettings: ignoring unsupported data type {}", setting_type);
        break;
    };
  }
...
u32 XamUserIsOnlineEnabled_entry(u32 user_index) {
  if (!IsSingleUserSignedIn(user_index)) {
    return 0;
  }
  return REX_KERNEL_STATE()->user_profile()->is_live_enabled() ? 1 : 0;
}
...
u32 XamUserGetMembershipTier_entry(u32 user_index) {
  ...
  return REX_KERNEL_STATE()->user_profile()->subscription_tier();
}
...
u32 XamUserGetMembershipTierFromXUID_entry(u64 xuid) {
  ...
  return user_profile->subscription_tier();
}

u32 XamUserGetCachedUserFlags_entry(u32 user_index) {
  return IsSingleUserSignedIn(user_index) ? GetSingleUserCachedFlags() : 0;
}

u32 XamUserGetUserFlags_entry(u32 user_index) {
  return XamUserGetCachedUserFlags_entry(user_index);
}

u32 XamUserGetUserFlagsFromXUID_entry(u64 xuid) {
  return xuid == REX_KERNEL_STATE()->user_profile()->xuid() ? GetSingleUserCachedFlags() : 0;
}
...
u32 XamUserIsParentalControlled_entry(u32 user_index) {
  ...
  return REX_KERNEL_STATE()->user_profile()->is_parental_controlled() ? 1 : 0;
}

u32 XamUserGetUserTenure_entry(u32 user_index, mapped_u32 tenure_level_ptr,
                                mapped_u32 milestone_ptr, mapped_u64 milestone_date_ptr,
                                u32 overlap_ptr) {
  (void)overlap_ptr;
  if (!IsSingleUserSignedIn(user_index)) {
    return X_E_INVALIDARG;
  }
  if (tenure_level_ptr) *tenure_level_ptr = 0;
  if (milestone_ptr) *milestone_ptr = 0;
  if (milestone_date_ptr) *milestone_date_ptr = 0;
  return X_ERROR_SUCCESS;
}
...
REX_EXPORT(__imp__XamUserGetUserFlagsFromXUID,
           rex::kernel::xam::XamUserGetUserFlagsFromXUID_entry)
...
REX_EXPORT(__imp__XamUserIsParentalControlled,
           rex::kernel::xam::XamUserIsParentalControlled_entry)
