...
void XamShowDirtyDiscErrorUI_entry(u32 user_index) {
  // GHWOR DIRTY DISC CALLER PROBE
  uint32_t ghwor_dirty_lr = 0;
  uint32_t ghwor_dirty_callsite = 0;

  auto* ghwor_dirty_thread =
      rex::system::XThread::GetCurrentThread();

  if (ghwor_dirty_thread &&
      ghwor_dirty_thread->thread_state() &&
      ghwor_dirty_thread->thread_state()->context()) {

    auto* ghwor_dirty_ctx =
        ghwor_dirty_thread->thread_state()->context();
    ...
    if (ghwor_dirty_lr >= 4) {
      ghwor_dirty_callsite =
          ghwor_dirty_lr - 4;
    }
  }

  REXKRNL_ERROR(
      "GHWOR DIRTYDISC CALLER: lr={:08X} callsite={:08X}",
      ghwor_dirty_lr,
      ghwor_dirty_callsite);

  // GHWOR DIRTY DISC PARENT CALLER PROBE
  uint32_t ghwor_dirty_sp = 0;
  uint32_t ghwor_dirty_backchain = 0;
  uint32_t ghwor_dirty_parent_lr = 0;
  uint32_t ghwor_dirty_parent_callsite = 0;

  if (ghwor_dirty_thread &&
      ghwor_dirty_thread->thread_state() &&
      ghwor_dirty_thread->thread_state()->context()) {

    auto* ghwor_parent_ctx =
        ghwor_dirty_thread->thread_state()->context();

    ghwor_dirty_sp =
        static_cast<uint32_t>(
            ghwor_parent_ctx->r1.u64);

    auto* ghwor_backchain_ptr =
        REX_KERNEL_MEMORY()->TranslateVirtual<be<uint32_t>*>(
            ghwor_dirty_sp);

    if (ghwor_backchain_ptr) {
      ghwor_dirty_backchain =
          static_cast<uint32_t>(
              *ghwor_backchain_ptr);
    }

    auto* ghwor_parent_lr_ptr =
        REX_KERNEL_MEMORY()->TranslateVirtual<be<uint32_t>*>(
            ghwor_dirty_sp + 0x58);

    if (ghwor_parent_lr_ptr) {
      ghwor_dirty_parent_lr =
          static_cast<uint32_t>(
              *ghwor_parent_lr_ptr);

      if (ghwor_dirty_parent_lr >= 4) {
        ghwor_dirty_parent_callsite =
            ghwor_dirty_parent_lr - 4;
      }
    }
  }

  REXKRNL_ERROR(
      "GHWOR DIRTYDISC PARENT: sp={:08X} backchain={:08X} lr={:08X} callsite={:08X}",
      ghwor_dirty_sp,
      ghwor_dirty_backchain,
      ghwor_dirty_parent_lr,
      ghwor_dirty_parent_callsite);

  if (ghwor_dirty_parent_callsite) {
    uint32_t ghwor_parent_dump_start =
        ghwor_dirty_parent_callsite;

    if (ghwor_parent_dump_start >= 0x100) {
      ghwor_parent_dump_start -= 0x100;
    }

    const uint32_t ghwor_parent_dump_end =
        ghwor_dirty_parent_callsite + 0x40;

    REXKRNL_ERROR(
        "GHWOR DIRTYDISC PARENT PPC BEGIN: start={:08X} end={:08X}",
        ghwor_parent_dump_start,
        ghwor_parent_dump_end);

    for (uint32_t ghwor_addr = ghwor_parent_dump_start;
         ghwor_addr <= ghwor_parent_dump_end;
         ghwor_addr += 4) {
      ...
      if (ghwor_insn_ptr) {
        const uint32_t ghwor_insn =
            static_cast<uint32_t>(
                *ghwor_insn_ptr);

        REXKRNL_ERROR(
            "GHWOR DIRTYDISC PARENT PPC: {:08X} {:08X}",
            ghwor_addr,
            ghwor_insn);
      }
      ...
      if (!ghwor_insn_ptr) {
        REXKRNL_ERROR(
            "GHWOR DIRTYDISC PARENT PPC: {:08X} UNMAPPED",
            ghwor_addr);
      }
    }

    REXKRNL_ERROR(
        "GHWOR DIRTYDISC PARENT PPC END");
  }
  // GHWOR DIRTY DISC TRIGGER FUNCTION PROBE
  if (ghwor_dirty_thread &&
      ghwor_dirty_thread->thread_state() &&
      ghwor_dirty_thread->thread_state()->context()) {

    auto* ghwor_trigger_ctx =
        ghwor_dirty_thread->thread_state()->context();

    const uint32_t ghwor_r26 =
        static_cast<uint32_t>(ghwor_trigger_ctx->r26.u64);

    const uint32_t ghwor_r29 =
        static_cast<uint32_t>(ghwor_trigger_ctx->r29.u64);

    const uint32_t ghwor_r30 =
        static_cast<uint32_t>(ghwor_trigger_ctx->r30.u64);

    const uint32_t ghwor_r31 =
        static_cast<uint32_t>(ghwor_trigger_ctx->r31.u64);

    REXKRNL_ERROR(
        "GHWOR DIRTYDISC TRIGGER REGS: r26={:08X} r29={:08X} r30={:08X} r31={:08X}",
        ghwor_r26,
        ghwor_r29,
        ghwor_r30,
        ghwor_r31);
    ...
    if (ghwor_r31) {
      ...
      for (uint32_t ghwor_offset : ghwor_offsets) {
        auto* ghwor_field =
            REX_KERNEL_MEMORY()->TranslateVirtual<be<uint32_t>*>(
                ghwor_r31 + ghwor_offset);

        if (ghwor_field) {
          REXKRNL_ERROR(
              "GHWOR DIRTYDISC R31 FIELD: offset={:03X} value={:08X}",
              ghwor_offset,
              static_cast<uint32_t>(*ghwor_field));
        }
      }
    }

    REXKRNL_ERROR(
        "GHWOR DIRTYDISC TRIGGER FUNC BEGIN: start=8256C440 end=8256C540");

    for (uint32_t ghwor_addr = 0x8256C440;
         ghwor_addr <= 0x8256C540;
         ghwor_addr += 4) {
      ...
      if (ghwor_insn) {
        REXKRNL_ERROR(
            "GHWOR DIRTYDISC TRIGGER FUNC: {:08X} {:08X}",
            ghwor_addr,
            static_cast<uint32_t>(*ghwor_insn));
      }

      if (!ghwor_insn) {
        REXKRNL_ERROR(
            "GHWOR DIRTYDISC TRIGGER FUNC: {:08X} UNMAPPED",
            ghwor_addr);
      }
    }

    REXKRNL_ERROR(
        "GHWOR DIRTYDISC TRIGGER FUNC END");

    REXKRNL_ERROR(
        "GHWOR DIRTYDISC STUB BEGIN: start=8242C3C0 end=8242C440");

    for (uint32_t ghwor_addr = 0x8242C3C0;
         ghwor_addr <= 0x8242C440;
         ghwor_addr += 4) {
      ...
      if (!ghwor_insn) {
        REXKRNL_ERROR(
            "GHWOR DIRTYDISC STUB: {:08X} UNMAPPED",
            ghwor_addr);
      }
    }

    REXKRNL_ERROR(
        "GHWOR DIRTYDISC STUB END");
  }
  // GHWOR DIRTY DISC STATE HELPER PROBE
  if (ghwor_dirty_thread &&
      ghwor_dirty_thread->thread_state() &&
      ghwor_dirty_thread->thread_state()->context()) {

    auto* ghwor_state_ctx =
        ghwor_dirty_thread->thread_state()->context();

    const uint32_t ghwor_state_r31 =
        static_cast<uint32_t>(
            ghwor_state_ctx->r31.u64);

    const uint32_t ghwor_state_base =
        ghwor_state_r31 + 0x114;

    REXKRNL_ERROR(
        "GHWOR DIRTYDISC STATE BASE: r31={:08X} state={:08X}",
        ghwor_state_r31,
        ghwor_state_base);

    for (uint32_t ghwor_off = 0;
         ghwor_off <= 0x100;
         ghwor_off += 4) {

      auto* ghwor_state_ptr =
          REX_KERNEL_MEMORY()->TranslateVirtual<be<uint32_t>*>(
              ghwor_state_base + ghwor_off);

      if (ghwor_state_ptr) {
        REXKRNL_ERROR(
            "GHWOR DIRTYDISC STATE MEM: +{:03X} {:08X}",
            ghwor_off,
            static_cast<uint32_t>(*ghwor_state_ptr));
      }

      if (!ghwor_state_ptr) {
        REXKRNL_ERROR(
            "GHWOR DIRTYDISC STATE MEM: +{:03X} UNMAPPED",
            ghwor_off);
      }
    }

    REXKRNL_ERROR(
        "GHWOR DIRTYDISC STATE HELPER BEGIN: start=8256BE80 end=8256C100");

    for (uint32_t ghwor_addr = 0x8256BE80;
         ghwor_addr <= 0x8256C100;
         ghwor_addr += 4) {
      ...
      if (ghwor_insn) {
        REXKRNL_ERROR(
            "GHWOR DIRTYDISC STATE HELPER: {:08X} {:08X}",
            ghwor_addr,
            static_cast<uint32_t>(*ghwor_insn));
      }
      ...
      if (!ghwor_insn) {
        REXKRNL_ERROR(
            "GHWOR DIRTYDISC STATE HELPER: {:08X} UNMAPPED",
            ghwor_addr);
      }
    }

    REXKRNL_ERROR(
        "GHWOR DIRTYDISC STATE HELPER END");
  }
  // GHWOR DIRTY DISC PPC MEMORY DUMP
  if (ghwor_dirty_callsite) {
    const uint32_t ghwor_dump_start =
        ghwor_dirty_callsite - 0x80;

    const uint32_t ghwor_dump_end =
        ghwor_dirty_callsite + 0x20;

    REXKRNL_ERROR(
        "GHWOR DIRTYDISC PPC BEGIN: start={:08X} end={:08X}",
        ghwor_dump_start,
        ghwor_dump_end);

    for (uint32_t ghwor_addr = ghwor_dump_start;
         ghwor_addr <= ghwor_dump_end;
         ghwor_addr += 4) {
      ...
      if (ghwor_insn_ptr) {
        const uint32_t ghwor_insn =
            static_cast<uint32_t>(*ghwor_insn_ptr);

        REXKRNL_ERROR(
            "GHWOR DIRTYDISC PPC: {:08X} {:08X}",
            ghwor_addr,
            ghwor_insn);
      }
      ...
      if (!ghwor_insn_ptr) {
        REXKRNL_ERROR(
            "GHWOR DIRTYDISC PPC: {:08X} UNMAPPED",
            ghwor_addr);
      }
    }

    REXKRNL_ERROR("GHWOR DIRTYDISC PPC END");
  }
