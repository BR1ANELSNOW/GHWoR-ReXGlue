...
bool ReXApp::SetupEnvironment() {
  ...
  // The per-title TOML must be loaded before resolving paths. Previously the
  // file was parsed only after game_data_root, user_data_root, cache_root and
  // related values had already been copied into PathConfig, so the log claimed
  // the config was loaded while those settings were silently ignored for the
  // current launch.
  // GHWoR SDK has one authoritative, portable per-title configuration.
  config_path_ = exe_dir / "ghwor.toml";
  if (std::filesystem::exists(config_path_)) {
    rex::cvar::LoadConfig(config_path_);
  }

  // Portable per-title configs use paths relative to the executable, not to
  // whatever working directory Explorer, a shortcut or a launcher selected.
  const auto resolve_configured_path = [&exe_dir](std::filesystem::path path) {
    return path.empty() || path.is_absolute() ? path : (exe_dir / path).lexically_normal();
  };

  std::filesystem::path game_dir = (exe_dir / "game").lexically_normal();
  std::string game_data_cvar = REXCVAR_GET(game_data_root);
  if (!game_data_cvar.empty()) {
    game_dir = resolve_configured_path(game_data_cvar);
  }
  ...
  if (!user_data_cvar.empty()) {
    user_dir = resolve_configured_path(user_data_cvar);
  } else {
    user_dir = rex::filesystem::GetUserFolder() / "Guitar Hero - Warriors of Rock";
  }
  ...
  if (!update_data_cvar.empty()) {
    update_dir = resolve_configured_path(update_data_cvar);
  }
  ...
  if (!cache_root_cvar.empty()) {
    cache_dir = resolve_configured_path(cache_root_cvar);
  } else {
  ...
  if (!metadata_root_cvar.empty()) {
    metadata_dir = resolve_configured_path(metadata_root_cvar);
  }
...
bool ReXApp::ConstructRuntime(const PathConfig& paths) {
  ...
  // The runtime, kernel, audio backend and GPU plugin have now registered all
  // of their cvars. Report unknown TOML keys and the exact effective values.
  // Keeping this in ConstructRuntime also covers the asynchronous path wizard.
  rex::cvar::FinalizeInit();
  rex::cvar::LogEffectiveConfig();
...
void ReXApp::SetupOverlays(rex::ui::Presenter* presenter, rex::ui::ImmediateDrawer* drawer) {
  ...
  rex::ui::RegisterBind(
      "bind_install_content", "F8", "Install Xbox 360 content package", [this] {
#ifdef _WIN32
        auto* kernel_state = runtime_ ? runtime_->kernel_state() : nullptr;

        if (!kernel_state || !kernel_state->content_manager()) {
          REXLOG_ERROR(
              "Install Content: runtime/kernel/content manager not ready");
          return;
        }

        wchar_t file_path[32768] = {};

        static const wchar_t kContentFilter[] =
            L"Xbox 360 content package\0*\0All files\0*.*\0";

        OPENFILENAMEW ofn{};
        ofn.lStructSize = sizeof(ofn);
        ofn.hwndOwner = nullptr;
        ofn.lpstrFilter = kContentFilter;
        ofn.lpstrFile = file_path;
        ofn.nMaxFile =
            static_cast<DWORD>(sizeof(file_path) / sizeof(file_path[0]));
        ofn.lpstrTitle = L"Install Xbox 360 Content - Select one or more packages";
        ...
        if (!GetOpenFileNameW(&ofn)) {
          ...
          if (dialog_error != 0) {
            REXLOG_ERROR(
                "Install Content: file dialog failed with 0x{:08X}",
                dialog_error);
          }
        ...
        auto install_package =
            [&](const std::filesystem::path& package_path) {
              REXLOG_INFO(
                  "Install Content: selected '{}'",
                  rex::path_to_utf8(package_path));

              const auto result =
                  content_manager->InstallContent(package_path);
              ...
              if (result == 0) {
                ...
                REXLOG_ERROR(
                    "Install Content: failed for '{}' with X_RESULT 0x{:08X}",
                    rex::path_to_utf8(package_path),
                    static_cast<uint32_t>(result));
              }
            };

        // GetOpenFileNameW + OFN_ALLOWMULTISELECT returns:
        //
        // Single file:
        //   C:\dir\file
        //
        // Multiple files:
        //   C:\dir\0file1\0file2\0...\0\0
        //
        // OFN_EXPLORER gives us this null-separated format.
        const wchar_t* next =
            file_path + lstrlenW(file_path) + 1;

        if (*next == L'\0') {
          // Single selection: file_path already contains the full path.
          install_package(
              std::filesystem::path(file_path));
        } else {
          // Multiple selection: first string is the directory,
          // following strings are the selected filenames.
          const std::filesystem::path directory(file_path);

          while (*next != L'\0') {
            install_package(
                directory / std::filesystem::path(next));
        ...
        if (failed_count == 0) {
          MessageBoxW(
              nullptr,
              L"All selected Xbox 360 content packages were installed "
              L"successfully.\n\n"
              L"Restart GHWoR so the game enumerates the new content.",
              L"GHWoR - Install Content",
              MB_OK | MB_ICONINFORMATION);
        } else {
          MessageBoxW(
              nullptr,
              L"One or more Xbox 360 content packages could not be "
              L"installed.\n\n"
              L"The successful packages were kept installed.\n"
              L"Check the log for the X_RESULT error codes.",
              L"GHWoR - Install Content",
              MB_OK | MB_ICONWARNING);
        }
#else
        REXLOG_ERROR(
            "Install Content is currently implemented only for Windows");
#endif
      });
...
void ReXApp::OnKeyDown(ui::KeyEvent& e) {
  ...
  // GHWoR: reserve bare F1-F12 for gameplay keyboard bindings.
  // ReXGlue system actions bound to F1-F12 only trigger while LEFT Shift is held.
  const uint16_t ghwor_vk = static_cast<uint16_t>(e.virtual_key());
  const uint16_t ghwor_f1 = static_cast<uint16_t>(ui::VirtualKey::kF1);
  const uint16_t ghwor_f12 = static_cast<uint16_t>(ui::VirtualKey::kF12);
  const bool ghwor_is_function_key = ghwor_vk >= ghwor_f1 && ghwor_vk <= ghwor_f12;
  const bool ghwor_shift_down = (GetAsyncKeyState(VK_LSHIFT) & 0x8000) != 0;

  static bool ghwor_fn_guard_logged = false;
  if (!ghwor_fn_guard_logged) {
    REXLOG_INFO("GHWOR SYSTEM FN LSHIFT GUARD v2 ACTIVE: bare F1-F12=gameplay LeftShift+Fn=ReXGlue");
    ghwor_fn_guard_logged = true;
  }

  if (ghwor_is_function_key && !ghwor_shift_down) {
    return;
  }
#endif
...
void ReXApp::OnDestroy() {
  ...
  rex::ui::UnregisterBind("bind_install_content");
