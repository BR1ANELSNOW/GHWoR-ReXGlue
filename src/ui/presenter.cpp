...
switch (paint_mode_) {
      ...
      case PaintMode::kUIThreadOnRequest:
        // GHWoR FramePacing A/B:
        // Keep guest-output painting on the normal UI/DXGI VBlank tick
        // instead of forcing WaitForUITickFromUIThread to early-out.
        RequestPaintOrConnectionRecoveryViaWindow(true);
        break;
...
#if REX_PLATFORM_WIN32
  std::scoped_lock<std::mutex> dxgi_ui_tick_lock(dxgi_ui_tick_mutex_);
  // Exact Xenia Canary cdd6049 behavior: mark the forced tick, but do not
  // wake the DXGI VBlank waiter here. The window paint request and the normal
  // VBlank tick provide the wake-up without bypassing host frame pacing.
