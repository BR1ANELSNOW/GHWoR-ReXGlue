...
X_STATUS XObject::WaitMultiple(uint32_t count, XObject** objects, uint32_t wait_type,
  ...
  XThread::CheckTitleTermination();
  if (wait_type) {
    auto result = rex::thread::WaitAny(std::move(wait_handles), alertable ? true : false, timeout_ms);
    XThread::CheckTitleTermination();
    switch (result.first) {
      case rex::thread::WaitResult::kSuccess:
    ...
    auto result = rex::thread::WaitAll(std::move(wait_handles), alertable ? true : false, timeout_ms);
    XThread::CheckTitleTermination();
    switch (result) {
      case rex::thread::WaitResult::kSuccess:
        for (uint32_t i = 0; i < count; i++) {
          objects[i]->WaitCallback();
          boost_increment = std::max(boost_increment, objects[i]->priority_increment());
        }
