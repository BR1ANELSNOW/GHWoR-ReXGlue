...
void UserProfile::LoadSetting(UserProfile::Setting* setting) {
  if (setting->is_title_specific()) {
    // A title-specific value from another game must never leak into the
    // current title if the current title has no saved value.
    ...
    // Cache the attempted title too. Missing settings should not hit disk on
    // every read until the title changes.
...
void UserProfile::SaveSetting(UserProfile::Setting* setting) {
  if (setting->is_title_specific()) {
    ...
    if (!file) {
      REXSYS_ERROR("Unable to save profile setting {:08X}", setting->setting_id);
      return;
    }
    if (!serialized_setting.empty()) {
