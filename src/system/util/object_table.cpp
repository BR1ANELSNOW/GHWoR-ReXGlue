...
X_STATUS ObjectTable::AddHandle(XObject* object, X_HANDLE* out_handle) {
  ...
  uint32_t handle = 0;
  {
    ...
    // GHWOR HANDLE ROTATION A/B
    //
    // Advance the allocation cursor after every successful allocation.
    // This avoids immediately reusing a recently released numeric handle
    // while asynchronous guest work may still contain the older value.
    if (XSUCCEEDED(result) && table_capacity_ > 1) {
      last_free_entry_ = slot + 1;

      if (last_free_entry_ >= table_capacity_) {
        last_free_entry_ = 1;
      }
    }
