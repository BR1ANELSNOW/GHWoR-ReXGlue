...
bool Memory::Initialize() {
  ...
  REXSYS_INFO("SUPERGLUEXENIA MEMORY V2 ACTIVE: cdd6049 free-block allocator + host-page policy");
...
static inline bool ShouldSkipHostCommit([[maybe_unused]] const BaseHeap& heap) {
  // cdd6049: on hosts with pages larger than 4 KiB, the file mapping is already
  // RW and committing 4 KiB guest sub-pages via mprotect/VirtualProtect is not
  // representable. Skip the redundant host commit and keep guest page state.
  return rex::memory::page_size() > 0x1000;
}
...
void BaseHeap::RebuildFreeBlocks() {
  ...
  for (const auto& free_block : free_blocks_) {
    unreserved_page_count_ += free_block.second;
  }
}
...
void BaseHeap::RemoveFreeBlock(uint32_t start_page, uint32_t page_count) {
  ...
  assert_true(start_page >= block_start && start_page + page_count <= block_end);
...
bool BaseHeap::Alloc(uint32_t size, uint32_t alignment, uint32_t allocation_type, uint32_t protect,
  ...
  *out_address = 0;
  ...
  // cdd6049: exclude the top 240 MB only from the 64 KiB guest virtual
  // heap to protect the title thread-stack region (0x70000000-0x7F000000).
  uint32_t heap_virtual_guest_offset = 0;
  if (heap_type_ == memory::HeapType::kGuestVirtual && page_size_ == 0x10000) {
...
bool BaseHeap::AllocFixed(uint32_t base_address, uint32_t size, uint32_t alignment,
  ...
  alignment = rex::round_up(alignment, page_size_);
  if ((base_address + host_address_offset_) % alignment != 0) {
    ...
    // ReXGlue maps the E0 physical heap with a host offset on Windows. Keep the
    // fixed guest address page-aligned if the stricter host alignment cannot be
  ...
  if (!page_count || start_page_number >= page_table_.size() ||
  ...
  const bool is_pure_reserve = allocation_type == memory::kMemoryAllocationReserve;
  for (uint32_t page_number = start_page_number; page_number <= end_page_number; ++page_number) {
    ...
    if ((allocation_type == memory::kMemoryAllocationReserve) && state) {
      REXSYS_ERROR("BaseHeap::AllocFixed attempting to reserve an already reserved range");
      return false;
    }
  ...
  if (allocation_type != memory::kMemoryAllocationReserve) {
    ...
    if (!ShouldSkipHostCommit(*this)) {
      ...
      result = rex::memory::AllocFixed(result, page_count << page_size_shift_, alloc_type,
  ...
  for (uint32_t page_number = start_page_number; page_number <= end_page_number; ++page_number) {
    ...
    if (!(page_entry.state & memory::kMemoryAllocationReserve)) {
      had_free_pages = true;
...
bool BaseHeap::AllocRange(uint32_t low_address, uint32_t high_address, uint32_t size,
  ...
  *out_address = 0;
  ...
  if (high_address < low_address || !page_count) {
  ...
  if (high_page_number < low_page_number ||
      page_count > (high_page_number - low_page_number + 1)) {
  ...
  if (top_down) {
    ...
    while (it != free_blocks_.begin()) {
      ...
      uint32_t block_start = it->first;
      uint32_t block_count = it->second;
      uint32_t block_end = block_start + block_count;  // exclusive
      ...
      uint32_t range_end_exclusive = high_page_number + 1;
      uint32_t usable_end = std::min(block_end, range_end_exclusive);
      ...
      if (latest_start >= usable_start && latest_start + page_count <= block_end &&
          latest_start + page_count <= range_end_exclusive) {
  ...
  if (!top_down) {
    ...
    for (; it != free_blocks_.end(); ++it) {
      ...
      uint32_t aligned_start = rex::round_up(earliest, page_scan_stride, false);
      if (aligned_start + page_count <= block_end &&
          aligned_start + page_count <= high_page_number + 1) {
...
bool BaseHeap::Protect(uint32_t address, uint32_t size, uint32_t protect,
  ...
  if (end_page_number >= page_table_.size()) {
    REXSYS_ERROR(
        "BaseHeap::Protect failed due to out-of-bounds range ({:08X} bytes from {:08x})", size,
        address);
    return false;
  }
  ...
  uint32_t host_page_size = static_cast<uint32_t>(rex::memory::page_size());
  uint32_t page_size_mask = host_page_size - 1;
  ...
  if (page_size_ == host_page_size ||
      (host_offset_aligned && (((page_count << page_size_shift_) & page_size_mask) == 0) &&
  ...
  if (page_size_ != host_page_size &&
      !(host_offset_aligned && (((page_count << page_size_shift_) & page_size_mask) == 0) &&
        (((start_page_number << page_size_shift_) & page_size_mask) == 0))) {
    if (page_size_ < host_page_size) {
      uint32_t start_offset = host_address_offset_ + (start_page_number << page_size_shift_);
      uint32_t end_offset =
      ...
      for (uint32_t host_offset = aligned_start_offset; host_offset < aligned_end_offset;
           host_offset += host_page_size) {
        uint32_t first_guest_page = 0;
        if (host_offset > host_address_offset_) {
          first_guest_page = (host_offset - host_address_offset_) >> page_size_shift_;
        }
        uint32_t host_page_end = host_offset + host_page_size - 1;
        ...
        for (uint32_t p = first_guest_page; p <= last_guest_page; ++p) {
          uint32_t page_protect =
              (p >= start_page_number && p <= end_page_number) ? protect : page_table_[p].current_protect;
          rex::memory::PageAccess page_access = ToPageAccess(page_protect);
          if (page_access == rex::memory::PageAccess::kReadWrite) {
            host_access = rex::memory::PageAccess::kReadWrite;
            break;
          }
          if (page_access == rex::memory::PageAccess::kReadOnly &&
              host_access == rex::memory::PageAccess::kNoAccess) {
            host_access = rex::memory::PageAccess::kReadOnly;
          }
        }

        rex::memory::Protect(
            reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(membase_) + heap_base_ + host_offset),
    ...
    if (page_size_ > host_page_size) {
      REXSYS_WARN("BaseHeap::Protect: unaligned to host page size; skipping host protection");
      if (old_protect) {
        *old_protect = page_table_[start_page_number].current_protect;
      }
      return false;
