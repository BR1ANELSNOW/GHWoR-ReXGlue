...
KernelState::KernelState(Runtime* emulator)
  ...
  InitializeProcess(&globals->title_process, X_PROCTYPE_TITLE, 10, 13, 17);
}

void KernelState::InitializeProcess(X_KPROCESS* process, uint32_t process_type, uint8_t priority_class,
                                    uint8_t default_priority, uint8_t max_dynamic_priority) {
...
void KernelState::SetExecutableModule(object_ref<UserModule> module) {
  ...
  pib->disable_quantum_decay = 0x06;
  ...
  if (!dispatch_thread_running_) {
    ...
    dispatch_thread_ = object_ref<XHostThread>(new XHostThread(this, 128 * 1024, 0, [this]() {
      // This host worker executes guest callbacks, so unlike normal host
      // threads it must be suspendable by the debugger.
...
void KernelState::EmulateCPInterruptDPC(uint32_t interrupt_callback,
  ...
  auto* ctx = thread->thread_state()->context();
  auto* pcr = memory_->TranslateVirtual<X_KPCR*>(
      static_cast<uint32_t>(ctx->r13.u64));

  DPCImpersonationScope dpc_scope = BeginDPCImpersonation();
  // Xenia Canary exposes the title process type only for the duration of the
  // emulated CP DPC. KeSetCurrentProcessType itself doesn't mutate the global
  // title process.
  pcr->processtype_value_in_dpc = X_PROCTYPE_TITLE;

  uint64_t args[] = {source, interrupt_callback_data};
  function_dispatcher_->ExecuteInterrupt(thread->thread_state(),
                                         interrupt_callback, args,
                                         rex::countof(args));

  pcr->processtype_value_in_dpc = X_PROCTYPE_IDLE;
  EndDPCImpersonation(dpc_scope);
}

void KernelState::DecayThreadPriorities() {
  auto global_lock = global_critical_region_.Acquire();
  for (auto& [id, thread] : threads_by_id_) {
