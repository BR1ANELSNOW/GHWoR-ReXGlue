...
void XThread::InitializeGuestObject() {
  ...
  // Mirror the Xenon scheduler fields from the owning process.
  if (process_ptr) {
    auto target_process = memory()->TranslateVirtual<X_KPROCESS*>(process_ptr);
    guest_thread->process_priority_class = target_process->process_priority_class;
    guest_thread->base_priority = target_process->default_thread_priority;
    guest_thread->base_priority_copy = target_process->default_thread_priority;
    guest_thread->max_dynamic_priority = target_process->max_dynamic_priority;
    guest_thread->priority_decrement = 0;
    guest_thread->saturation_increment = 0;
    guest_thread->priority = target_process->default_thread_priority;
    guest_thread->quantum = target_process->quantum;
    priority_ = target_process->default_thread_priority;
    base_priority_ = priority_;
  }
  quantum_start_ms_ = rex::chrono::Clock::QueryHostUptimeMillis();
...
void XThread::CheckQuantumAndDecay() {
  if (REXCVAR_GET(ignore_thread_priorities) || priority_ >= 18) {
    return;
  }
  uint64_t now = rex::chrono::Clock::QueryHostUptimeMillis();
  ...
  boost_amount_ = 0;
  int32_t new_priority = std::max(base_priority_, priority_ - total_decay);
...
void XThread::BoostOnWake(int32_t increment) {
  ...
  if (apply_boost) {
    ...
    boosted = std::min(boosted, max_cap);
