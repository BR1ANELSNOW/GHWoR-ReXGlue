...
// Xenia Canary cdd6049 host timing controls. In ReXGlue these live in
// rexruntime.dll so they are registered in the same CVar registry that loads
// ghwor.toml even when the existing GHWoR.exe is intentionally kept intact.
REXCVAR_DEFINE_BOOL(win32_high_resolution_timer, true, "Win32",
                    "Request high-resolution timer from the NT kernel");
REXCVAR_DEFINE_BOOL(win32_mmcss, true, "Win32",
                    "Opt in to MMCSS scheduling for prioritized CPU access");
#endif
...
#if REX_PLATFORM_WIN32
  // Xenia performs these process-wide timing requests before the emulation
  // threads are created. ReXGlue keeps the host EXE frozen, so perform the
  // equivalent initialization here before the clock, audio and GPU workers.
  if (REXCVAR_GET(win32_high_resolution_timer)) {
    thread::RequestHighResolutionTimer();
  }
  if (REXCVAR_GET(win32_mmcss)) {
    thread::RequestMMCSS();
  }
  REXSYS_INFO("XENIA HOST TIMING ACTIVE: high_resolution_timer={} mmcss={}",
              REXCVAR_GET(win32_high_resolution_timer), REXCVAR_GET(win32_mmcss));
#endif
