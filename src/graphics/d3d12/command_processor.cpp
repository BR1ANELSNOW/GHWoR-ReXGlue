...
vertex_buffers_in_sync_[0] = 0;
  ...
  const uint32_t guest_output_width = uint32_t(swap_texture_desc.Width);
  const uint32_t guest_output_height = uint32_t(swap_texture_desc.Height);
  auto aspect = graphics_system_->GetScaledAspectRatio();

  presenter->RefreshGuestOutput(
      guest_output_width, guest_output_height, aspect.first, aspect.second,
      [this, &swap_texture_srv_desc, frontbuffer_format, swap_texture_resource,
       &swap_texture_desc, guest_output_width, guest_output_height](
...
bool D3D12CommandProcessor::IssueDraw(xenos::PrimitiveType primitive_type, uint32_t index_count,
  ...
  // Phase 5A: passive resource census of the exact draw visually confirmed
  // to provide the highway surface texture. No draw is skipped or modified.

  // Identify the shader register driving the highway texture scroll. The raw
  // vertex buffer is constant throughout gameplay, so sample only the used
  // float constants once per second and log registers whose values changed.

  // Phase 5D: keep the main GPU plugin small. The heavy D3D12 renderer is
  // loaded externally only after this gameplay-only draw has appeared.

  // Visual identification probe for the narrow gameplay-only render pass
  // discovered by Frame Census. Never suppress memexport or depth-only work.

  // Narrow Phase 4C probe: this exact shader pair is absent from menus and
  // appears at a stable cadence during gameplay. Limit the suppression to its
  // pitch 640 line-strip draws so unrelated scene geometry remains intact.

  // Broad, venue-independent visual classifier. Quad-list composition draws
  // at the native 1280 pitch are alternated in two-second windows. Anything
  // belonging to this class will visibly blink instead of merely disappearing.

  // Exact shader pair identified by the successful Phase 4D broad probe.
  // It dominates both menu and gameplay quad batches; the narrow blink tells
  // whether it owns the highway/HUD or merely unrelated 2D decoration.

  // Gameplay-only large quad batch first observed exactly at song start by
  // Phase 4D. Isolate it from the already identified menu/HUD shader.
...
bool D3D12CommandProcessor::BeginSubmission(bool is_guest_command) {
  ...
  if (is_opening_frame) {
    ...
    // Xenia-style bounded maintenance: avoid walking both maps on every frame.
...
void D3D12CommandProcessor::UpdateSystemConstantValues(
  ...
  if (edram_rov_used) {
    ...
    if (zpd_active_query_is_rov_ && zpd_active_query_index_ != UINT32_MAX &&
        zpd_host_query_pool_ && zpd_host_query_pool_->rov_initialized()) {
...
void D3D12CommandProcessor::EnsureZPDQueryResources() {
  ...
  if (zpd_query_pool_needs_rov_counter_ && !IsZPDQueryPoolReady()) {
    if (!command_list_2_) {
      REXGPU_WARN(
          "ZPD/D3D12: ROV counter unavailable because CommandList2 is not available; "
          "keeping counter index sentinel active");
    } else {
      REXGPU_WARN(
          "ZPD/D3D12: ROV counter resources unavailable; keeping counter index sentinel active");
    }
  }
}
...
bool D3D12CommandProcessor::CloseZPDQuery(ReportHandle report_handle,
  ...
  if (!zpd_host_query_pool_ || zpd_active_query_index_ == UINT32_MAX) {
    return false;
  }
  if (zpd_active_query_is_rov_) {
