...
REXCVAR_DEFINE_BOOL(ghwor_joystick_enabled, false, "Input/GHWoR", "Enable selected SDL joystick mapper");
REXCVAR_DEFINE_INT32(ghwor_joystick_instrument, 0, "Input/GHWoR", "Use selected joystick as normal gamepad or Guitar Hero 5-lane drums").range(0, 1);
...
X_RESULT InputSystem::GetState(uint32_t user_index, X_INPUT_STATE* out_state) {
  ...
  for (auto& driver : drivers_) {
    ...
    if (result == X_ERROR_SUCCESS) {
      // Mapper enabled: selected SDL is first; do not merge its raw XInput twin.
      if (REXCVAR_GET(ghwor_joystick_enabled) && user_index == 0) {
...
std::unique_ptr<InputSystem> CreateDefaultInputSystem(bool tool_mode) {
  ...
  if (!tool_mode) {
    // GHWOR REAL JOYSTICK MAPPER v15A: selected SDL device gets first priority.
    if (REXCVAR_GET(ghwor_joystick_enabled)) {
      auto ghwor_sdl_driver = std::make_unique<sdl::SDLInputDriver>(nullptr, 0);
      if (ghwor_sdl_driver->Setup() == X_STATUS_SUCCESS) {
        input->AddDriver(std::move(ghwor_sdl_driver));
      }
    }
    ...
    if (!REXCVAR_GET(ghwor_joystick_enabled) && REXCVAR_GET(input_backend) == "sdl") {
