...
void MaybeYield() {
  // Xenia Canary cdd6049: Sleep(0) yields the remainder of the current
  // timeslice to any ready equal-priority thread, including on other CPUs.
  // SwitchToThread only considers the current processor and can devolve into
  // a hot spin when no suitable thread is ready there.
  ::Sleep(0);
}

void NanoSleep(int64_t ns) {
  if (ns == 0) {
    // Preserve Xenia's explicit zero-delay behavior.
  }

  using NtDelayExecutionFn = LONG(NTAPI*)(BOOLEAN, PLARGE_INTEGER);
  static NtDelayExecutionFn nt_delay_execution = []() -> NtDelayExecutionFn {
  ...
  if (!nt_delay_execution) {
    ...
    Sleep(std::chrono::microseconds(std::max<int64_t>(1, ns / 1000)));
    return;
  }

  // NtDelayExecution uses signed 100 ns intervals. A negative interval is
  // relative. Xenia rounds non-zero sub-100 ns requests up to one increment.
...
void RequestMMCSS() {
  ...
  using DwmEnableMMCSSFn = LONG(WINAPI*)(BOOL);
