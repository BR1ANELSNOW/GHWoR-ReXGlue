...
bool FunctionNode::tryResolveAsInternalLabel(uint32_t target) {
  // A local C++ label is only valid when the target is backed by a block that
  // will actually be emitted. PDATA/CONFIG bounds may contain unowned holes.
  if (!containsBlockAddress(target)) {
    return false;
  }
...
FunctionNode* FunctionGraph::getFunctionContainingBlock(uint32_t addr) {
  auto it = functionsByBase_.upper_bound(addr);
  while (it != functionsByBase_.begin()) {
    --it;
    if (it->second->containsBlockAddress(addr)) {
      return it->second;
    }
  }
  return nullptr;
...
const FunctionNode* FunctionGraph::getFunctionContainingBlock(uint32_t addr) const {
  auto it = functionsByBase_.upper_bound(addr);
  while (it != functionsByBase_.begin()) {
    --it;
    if (it->second->containsBlockAddress(addr)) {
...
TargetKind FunctionGraph::classifyTarget(uint32_t target, uint32_t callerAddr,
  ...
  // Case 4: Target is backed by an emitted block in the caller -> local label.
  // Do not use containsAddress() here: PDATA/CONFIG declared ranges may contain
  // holes, and emitting goto loc_X for a hole creates an undeclared C++ label.
  if (callerFn && callerFn->containsBlockAddress(target)) {
    return TargetKind::InternalLabel;
  }

  // Case 5: Target lies in another function's emitted block. Treat the exact
  // entry point separately above; an interior cross-function target must have
  // been resolved explicitly by the graph and is otherwise Unknown.
  //
  // Keeping this Unknown is intentional: Validate will reject it rather than
  // silently generating a local goto that cannot exist.
