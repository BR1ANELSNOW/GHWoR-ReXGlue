    ...
    for (const auto& block : node->blocks()) {
      ...
      for (size_t offset = 0; offset < block.size; offset += 4) {
        ...
        if (PPC_OP(insn) == PPC_OP_B && !PPC_BA(insn)) {
          ...
          // A branch is local only if its target is backed by a discovered
          // block that will actually be emitted. Declared PDATA/CONFIG bounds
          // alone are not enough because they may contain code/data holes.
          if (node->containsBlockAddress(target)) {
            isInternalJump = true;
            targetExists = true;
          }
          ...
          if (!targetExists) {
            const FunctionNode* containingFunc = graph.getFunctionContainingBlock(target);
            if (containingFunc) {
          ...
          if (!isInternalJump) {
            ...
            if (!edge) {
              // Code generation can only tail-call a target for which the graph
              // recorded a CallEdge. An interior address in another function is
              // not automatically callable and must not be accepted here.
              errors.Add(AnalysisErrors::Category::UnresolvedCall, target, site,
