...
void discoverFunction(CodegenContext& ctx, uint32_t funcAddr,
  ...
  // Register external call targets as new functions (existing behavior).
  ...
  // Register unconditional external branches as tail-call function candidates.
  // `discoverBlocks` has always populated `tailCalls`, but this phase used to
  // ignore that list entirely. Validate later requires every such target to
  // resolve to a function (or an address inside one), so valid PPC `b` tail
  // calls could survive all discovery phases and then fail as UnresolvedCall.
  // Do not split an already-known function merely because the branch lands at
  // an internal label; only create a new entry when no containing function is
  // known yet.
  for (uint32_t target : result.tailCalls) {
    if (graph.isEntryPoint(target) || graph.isImport(target)) {
      continue;
    }
    if (graph.getFunctionContainingBlock(target)) {
      continue;
    }
    if (binary.isInImportExportRange(target)) {
      continue;
  ...
  if (pdataSize > 0) {
    ...
    if (funcData) {
      for (uint32_t offset = 0; offset < pdataSize; offset += 4) {
        ...
        // Skip only branches to code that is actually part of this function.
        // The declared PDATA range may contain holes with no emitted block.
        if (!isCall && node->containsBlockAddress(target)) {
          continue;
        }

        graph.addUnresolvedJumpToFunction(funcAddr, site, target, isCall, false);

        // Calls and unconditional external branches are function-entry
        // candidates. The latter are tail calls and need the same registration
        // as normal `bl` targets so Validate can resolve the edge. Conditional
        // branches are left unchanged because they may target out-of-line
        // blocks rather than function entries.
        const bool isTailCall = !isCall && opcode == PPC_OP_B;
        if ((isCall || isTailCall) && !graph.isEntryPoint(target) && !graph.isImport(target) &&
            !graph.getFunctionContainingBlock(target)) {
          if (binary.isInImportExportRange(target)) {
            continue;
          }
          graph.addFunction(target, 4, FunctionAuthority::DISCOVERED, true);
