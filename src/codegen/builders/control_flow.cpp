...
bool build_b(BuilderContext& ctx) {
  ...
  switch (kind) {
    ...
    case TargetKind::Unknown:
      // Never turn a broad PDATA/CONFIG range hit into a local goto. A C++
      // label exists only for addresses backed by emitted blocks. Validate is
      // expected to reject unresolved targets before Write, but keep generated
      // code compile-safe if one slips through.
      REXCODEGEN_ERROR("Unresolved b target 0x{:08X} from 0x{:08X}", target, ctx.base);
      ctx.println("\tREX_FATAL(\"Unresolved branch from 0x{:08X} to 0x{:08X}\");", ctx.base,
                  target);
      ctx.println("\treturn;");
      break;
  }
