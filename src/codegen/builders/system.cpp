...
bool build_sync(BuilderContext& ctx) {
  // Xenia Canary cdd6049: PPC sync is a real memory barrier.
  ctx.println("\tstd::atomic_thread_fence(std::memory_order_seq_cst);");
  return true;
}

bool build_lwsync(BuilderContext& ctx) {
  // Keep the AOT ordering conservative and equivalent to Xenia's barrier path.
  ctx.println("\tstd::atomic_thread_fence(std::memory_order_seq_cst);");
  return true;
}

bool build_eieio(BuilderContext& ctx) {
  // Xenia Canary cdd6049 emits MemoryBarrier for eieio.
  ctx.println("\tstd::atomic_thread_fence(std::memory_order_seq_cst);");
  return true;
}
...
bool build_dcbf(BuilderContext& ctx) {
  // Xenia cdd6049 x64 backend maps Xenon dcbf to CLFLUSH on both 64-byte
  // host halves of the 128-byte Xenon cache line.
  ctx.print("	{} = (", ctx.ea());
  if (ctx.insn.operands[0] != 0) ctx.print("{}.u32 + ", ctx.r(ctx.insn.operands[0]));
  ctx.println("{}.u32) & ~127u;", ctx.r(ctx.insn.operands[1]));
  ctx.println("	_mm_clflush(reinterpret_cast<const char*>(REX_RAW_ADDR({})));", ctx.ea());
  ctx.println("	_mm_clflush(reinterpret_cast<const char*>(REX_RAW_ADDR({} + 64u)));", ctx.ea());
  return true;
}

bool build_dcbt(BuilderContext& ctx) {
  // Xenia cdd6049: prefetcht0 for the 128-byte Xenon cache line.
  ctx.print("	{} = (", ctx.ea());
  if (ctx.insn.operands[0] != 0) ctx.print("{}.u32 + ", ctx.r(ctx.insn.operands[0]));
  ctx.println("{}.u32) & ~127u;", ctx.r(ctx.insn.operands[1]));
  ctx.println("	_mm_prefetch(reinterpret_cast<const char*>(REX_RAW_ADDR({})), _MM_HINT_T0);", ctx.ea());
  ctx.println("	_mm_prefetch(reinterpret_cast<const char*>(REX_RAW_ADDR({} + 64u)), _MM_HINT_T0);", ctx.ea());
  return true;
}

bool build_dcbtst(BuilderContext& ctx) {
  // PREFETCHW is not guaranteed on every x64 target; T0 preserves Xenia's
  // prefetch semantics without introducing an unsupported instruction.
  ctx.print("	{} = (", ctx.ea());
  if (ctx.insn.operands[0] != 0) ctx.print("{}.u32 + ", ctx.r(ctx.insn.operands[0]));
  ctx.println("{}.u32) & ~127u;", ctx.r(ctx.insn.operands[1]));
  ctx.println("	_mm_prefetch(reinterpret_cast<const char*>(REX_RAW_ADDR({})), _MM_HINT_T0);", ctx.ea());
...
bool build_dcbz(BuilderContext& ctx) {
  // Xenon has no short cache line: normal dcbz clears 128 bytes (cdd6049).
  ctx.print("	{} = (", ctx.ea());
  if (ctx.insn.operands[0] != 0) ctx.print("{}.u32 + ", ctx.r(ctx.insn.operands[0]));
  ctx.println("{}.u32) & ~127u;", ctx.r(ctx.insn.operands[1]));
...
bool build_dcbzl(BuilderContext& ctx) {
  // Same 128-byte Xenon line semantics.
  ctx.print("	{} = (", ctx.ea());
  if (ctx.insn.operands[0] != 0) ctx.print("{}.u32 + ", ctx.r(ctx.insn.operands[0]));
  ctx.println("{}.u32) & ~127u;", ctx.r(ctx.insn.operands[1]));
  ctx.println("	memset((void*)REX_RAW_ADDR({}), 0, 128);", ctx.ea());
...
bool build_dcbst(BuilderContext& ctx) {
  // Xenia cdd6049 maps dcbst to the same host CLFLUSH path.
  ctx.print("	{} = (", ctx.ea());
  if (ctx.insn.operands[0] != 0) ctx.print("{}.u32 + ", ctx.r(ctx.insn.operands[0]));
  ctx.println("{}.u32) & ~127u;", ctx.r(ctx.insn.operands[1]));
  ctx.println("	_mm_clflush(reinterpret_cast<const char*>(REX_RAW_ADDR({})));", ctx.ea());
...
bool build_mfmsr(BuilderContext& ctx) {
  if (!ctx.config().skipMsr) {
    // Xenia cdd6049 returns the emulated MSR stored in PPCContext.
...
bool build_mtmsrd(BuilderContext& ctx) {
  if (!ctx.config().skipMsr) {
    // Xenia cdd6049: mtmsrd updates only the emulated EE bit (0x8000) under
    // the Xenon mask. Do not infer a host global lock from the source GPR.
    ctx.println("\tctx.msr = ({}.u32 & 0x8000u) | (ctx.msr & ~0x8000u);",
