...
inline void emitAtomicLoadReserve(BuilderContext& ctx, const char* ptr_type, const char* bswap_func,
  ...
  // Xenia cdd6049 issues a memory barrier before a reserved load.
  ctx.println("\tstd::atomic_thread_fence(std::memory_order_seq_cst);");
  ctx.println("\t{}.{} = *reinterpret_cast<volatile {}*>(REX_RAW_ADDR({}));", ctx.reserved(),
              reserved_field, ptr_type, ctx.ea());
...
inline void emitAtomicStoreConditional(BuilderContext& ctx, const char* ptr_type,
  ...
  // Xenia cdd6049 issues a memory barrier after a conditional store.
  ctx.println("\tstd::atomic_thread_fence(std::memory_order_seq_cst);");
}
