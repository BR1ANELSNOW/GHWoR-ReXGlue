...
// A PDATA/CONFIG FunctionNode may intentionally have a declared address range
// larger than the set of blocks that were actually discovered for it.  Using
// FunctionNode::containsAddress here would treat those holes as occupied code,
// which can discard a valid gap function.  Code generation can only emit local
// labels for addresses present in real blocks, so GapFill ownership must use
// block coverage rather than the broad declared range.
static bool IsCoveredByDiscoveredBlock(const FunctionNode* function, uint32_t address) {
  if (!function) {
    return false;
  }
...
void gapFillCodeRegions(CodegenContext& ctx) {
  ...
  for (const auto& region : scan.codeRegions) {
    ...
    for (const auto& segment : segments) {
      ...
      // Skip only when real discovered code covers this address.  PDATA and
      // CONFIG nodes may claim a wider declared range containing holes; those
      // holes must remain eligible for GapFill or direct branches can later be
      // emitted as gotos to labels that never exist.
      if (auto* containingFunc = graph.getFunctionContaining(segment.start)) {
        if (IsCoveredByDiscoveredBlock(containingFunc, segment.start)) {
          continue;
        }
      }
...
void cleanupAbsorbedGapFills(CodegenContext& ctx) {
  ...
  for (const auto& [addr, node] : graph.functions()) {
    ...
    for (const auto& [otherAddr, otherNode] : graph.functions()) {
      if (otherAddr == addr)
        continue;
      // A declared PDATA/CONFIG range is not enough to prove that this
      // GAP_FILL was absorbed.  Only remove it when another function has an
      // actual discovered block covering the entry address.
      if (!IsCoveredByDiscoveredBlock(otherNode.get(), addr))
        continue;

      // This GAP_FILL is inside another function's real discovered blocks
