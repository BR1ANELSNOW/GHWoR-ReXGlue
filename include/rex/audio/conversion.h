...
inline void sequential_6_BE_to_interleaved_2_LE(float* output, const float* input,
  ...
  const __m128i byte_swap_shuffle =
      _mm_set_epi8(12, 13, 14, 15, 8, 9, 10, 11, 4, 5, 6, 7, 0, 1, 2, 3);
  const __m128 front_gain = _mm_set1_ps(0.55f);
  const __m128 center_gain = _mm_set1_ps(0.20f);
  const __m128 rear_gain = _mm_set1_ps(0.25f);
  ...
  for (size_t sample = 0; sample < ch_sample_count; sample += 4) {
    ...
    __m128 left = _mm_add_ps(
        _mm_add_ps(_mm_mul_ps(fl, front_gain),
                   _mm_mul_ps(bl, rear_gain)),
        _mm_mul_ps(fc, center_gain));

    __m128 right = _mm_add_ps(
        _mm_add_ps(_mm_mul_ps(fr, front_gain),
                   _mm_mul_ps(br, rear_gain)),
        _mm_mul_ps(fc, center_gain));
