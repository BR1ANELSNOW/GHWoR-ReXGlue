...
class Clock {
 public:
  ...
  // Queries the tick count exposed specifically to recompiled game mftb
  // instructions. This may use a title-side correction without changing the
  // runtime clock used by vblank, audio and kernel scheduling.
  static uint64_t QueryGameTimebaseTickCount();
  // Diagnostic callsite variant. It preserves the current game timebase value
  // and only records the generated source location when enabled.
  static uint64_t QueryGameTimebaseTickCountAt(const char* source_file,
