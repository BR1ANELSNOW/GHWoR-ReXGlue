...
class FunctionGraph {
 public:
  ...
  // Get function containing address according to its declared ownership/range.
  FunctionNode* getFunctionContaining(uint32_t addr);
  const FunctionNode* getFunctionContaining(uint32_t addr) const;

  // Get function whose discovered/emittable blocks actually contain address.
  // This is the correct query for deciding whether a branch can become a C++
  // local label. PDATA/CONFIG ranges may contain holes with no emitted code.
  FunctionNode* getFunctionContainingBlock(uint32_t addr);
  const FunctionNode* getFunctionContainingBlock(uint32_t addr) const;
