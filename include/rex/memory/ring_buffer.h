...
class RingBuffer {
 public:
  ...
  ring_size_t read_count() const {
    ...
    if (read_offs <= write_offs) [[likely]] {
      return offset_delta;
    }
  ...
  // Xenia cdd6049: when a read wraps, prefetch the second range so the CPU
  // doesn't hit a cold line exactly at the ring boundary. Clang's locality=2
  // is used as the portable ReX equivalent of Xenia's Level2 software cache
  // prefetch tag.
  inline ReadRange BeginPrefetchedRead(size_t count) {
    ReadRange range = BeginRead(count);
    if (range.second) {
      constexpr ring_size_t kHostCacheLineSize = 64;
      ring_size_t num_lines =
          (range.second_length + kHostCacheLineSize - 1) / kHostCacheLineSize;
      for (ring_size_t i = 0; i < num_lines; ++i) {
        __builtin_prefetch(range.second + i * kHostCacheLineSize, 0, 2);
      }
    }
    return range;
  }
