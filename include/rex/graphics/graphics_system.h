...
class GraphicsSystem : public system::IGraphicsSystem {
 public:
  ...
  std::pair<uint32_t, uint32_t> GetScaledAspectRatio() const override {
    return {scaled_aspect_x_, scaled_aspect_y_};
  }
  void SetScaledAspectRatio(uint32_t x, uint32_t y) override {
    scaled_aspect_x_ = x;
    scaled_aspect_y_ = y;
  }
 ...
 protected:
  ...
  // Xenia cdd6049 defaults the hardware scaler output to 16:9.
