...
class UserProfile {
 public:
  ...
  struct Setting {
    ...
    template <typename T>
    static std::vector<uint8_t> SerializeScalar(const T& value) {
    ...
    template <typename T>
    static bool DeserializeScalar(const std::vector<uint8_t>& data, T& value) {
  ...
  struct Int32Setting : public Setting {
    ...
    std::vector<uint8_t> Serialize() const override { return SerializeScalar(value); }
    void Deserialize(std::vector<uint8_t> data) override {
      if (DeserializeScalar(data, value)) is_set = true;
    }
  };
  ...
  struct UnicodeSetting : public Setting {
    ...
    std::vector<uint8_t> Serialize() const override {
      std::vector<uint8_t> data(value.size() * sizeof(char16_t));
      if (!data.empty()) std::memcpy(data.data(), value.data(), data.size());
      return data;
    }
    void Deserialize(std::vector<uint8_t> data) override {
      if (data.size() % sizeof(char16_t)) return;
      value.resize(data.size() / sizeof(char16_t));
      if (!data.empty()) std::memcpy(value.data(), data.data(), data.size());
      is_set = true;
    }
  };
  ...
  uint32_t subscription_tier() const { return 6; /* Gold, matching legacy ReX behavior. */ }
  bool is_live_enabled() const { return true; }
  bool is_parental_controlled() const { return false; }
