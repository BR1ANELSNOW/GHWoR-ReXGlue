...
// Compatibility alias used by older ReXGlue code.
constexpr uint32_t X_PROCTYPE_USER = X_PROCTYPE_TITLE;
...
struct X_KPROCESS {
  ...
  uint8_t process_priority_class;             // 0x18
  uint8_t default_thread_priority;            // 0x19
  uint8_t max_dynamic_priority;               // 0x1A
  uint8_t disable_quantum_decay;              // 0x1B
