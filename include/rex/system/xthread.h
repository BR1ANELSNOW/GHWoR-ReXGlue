...
class XThread : public XObject {
 public:
  ...
  // Xenia Canary debugger suspension policy. Host-only threads default to
  // non-suspendable in XHostThread, while threads that may execute guest
  // callbacks can opt back in.
 ...
 protected:
  ...
  // Mirrors Xenia cpu::Thread::can_debugger_suspend_. ReXGlue has no
  // separate CPU Thread base, so keep the policy on XThread.
  ...
  int32_t boost_amount_ = 0;
  uint64_t quantum_start_ms_ = 0;
...
class XHostThread : public XThread {
 public:
  // Keep the Alpha 8.2 exported ABI. New callers use this inline adapter so
  // the GPU plugin imports only the compatible four-argument constructor.
  XHostThread(KernelState* kernel_state, uint32_t stack_size, uint32_t creation_flags,
              std::function<int()> host_fn);
  XHostThread(KernelState* kernel_state, uint32_t stack_size, uint32_t creation_flags,
              std::function<int()> host_fn, uint32_t guest_process)
      : XHostThread(kernel_state, stack_size, creation_flags, host_fn) {
    (void)guest_process;
  }
